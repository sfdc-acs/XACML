...
public class GsonJsonResponseMissingAttributeDetail {
	...
	public GsonJsonResponseMissingAttributeDetail(MissingAttributeDetail detail) {
	    ...
	    if (! detail.getAttributeValues().isEmpty()) {
	        this.value = new ArrayList<Object>(detail.getAttributeValues().size());
	        detail.getAttributeValues().forEach(val -> {
	            this.value.add(val.getValue());
	        });
	    }
	}
	...
	public MissingAttributeDetail extractXacml() {
        ...
        if (this.value != null) {
            this.value.forEach(val -> {
                mutableMissing.addAttributeValue(new StdAttributeValue<>(this.dataType, val));
            });
        }
